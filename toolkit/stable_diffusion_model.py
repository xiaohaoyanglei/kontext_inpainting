...
class StableDiffusion:
    ...
    def load_model(self):
        ...
        if self.model_config.is_xl or self.model_config.is_ssd or self.model_config.is_vega:
            ...
            # 🔥 FLUX Fill 模型适配 - 重新初始化x_embedder以支持inpainting
            import torch.nn as nn
            original_in_features = transformer.x_embedder.in_features
            expected_in_features = 64  # AI-toolkit inpainting的实际输入通道数
            
            if original_in_features != expected_in_features:
                print_acc(f"🔧 检测到FLUX Fill模型，重新初始化x_embedder")
                print_acc(f"   原始输入通道: {original_in_features} → 新输入通道: {expected_in_features}")
                
                # 获取原始输出维度
                out_features = transformer.x_embedder.out_features
                has_bias = transformer.x_embedder.bias is not None
                
                # 重新创建x_embedder
                new_x_embedder = nn.Linear(expected_in_features, out_features, bias=has_bias)
                
                # 初始化权重
                nn.init.trunc_normal_(new_x_embedder.weight, std=0.02)
                if has_bias:
                    nn.init.zeros_(new_x_embedder.bias)
                
                # 替换x_embedder
                transformer.x_embedder = new_x_embedder
                print_acc(f"✅ x_embedder重新初始化完成: {expected_in_features} → {out_features}")
