...
class ImageDataset(Dataset, CaptionMixin):
    def __init__(self, config, source_dir=None, target_dir=None, mask_dir=None):
        ...
        if source_dir is not None and target_dir is not None:
            ...
            self.path = source_dir  # 保持兼容性，使用source_dir作为主路径
            
            # 从source_dir获取文件列表
            self.file_list = [os.path.join(self.source_dir, file) for file in os.listdir(self.source_dir) if
        ...
        if self.mask_dir is not None:
            self.mask_map = {}
            mask_files = [f for f in os.listdir(self.mask_dir) if f.lower().endswith(('.png', '.jpg', '.jpeg', '.webp'))]
            for mf in mask_files:
                self.mask_map[os.path.splitext(mf)[0]] = os.path.join(self.mask_dir, mf)
        else:
            self.mask_map = None
        ...
        if self.target_dir is not None:
            self.target_map = {}
            target_files = [f for f in os.listdir(self.target_dir) if f.lower().endswith(('.png', '.jpg', '.jpeg', '.webp'))]
            for tf in target_files:
                self.target_map[os.path.splitext(tf)[0]] = os.path.join(self.target_dir, tf)
        else:
            self.target_map = None
        
        # 为CaptionMixin设置必要的属性
        if self.include_prompt:
            # 创建一个简单的dataset_config对象来支持CaptionMixin
            class SimpleDatasetConfig:
                def __init__(self, caption_ext, replacements=None):
                    self.caption_ext = caption_ext
                    self.replacements = replacements or []
            
            self.dataset_config = SimpleDatasetConfig(
                caption_ext=self.caption_type,
                replacements=self.get_config('replacements', [])
            )
    ...
    def __getitem__(self, index):
        ...
        # 加载source图像（用于生成带洞图像）
        try:
            source_img = exif_transpose(Image.open(source_path)).convert('RGB')
        except Exception as e:
            print_acc(f"Error opening source image: {source_path}")
            print_acc(e)
            source_img = Image.fromarray(np.random.randint(0, 255, (1024, 1024, 3), dtype=np.uint8))
        
        # 加载target图像（作为标准答案）
        target_tensor = None
        if self.target_map is not None:
            target_path = self.target_map.get(img_name, None)
            if target_path is not None:
                try:
                    target_img = exif_transpose(Image.open(target_path)).convert('RGB')
                    # 应用相同的预处理
                    target_img = target_img.resize((int(target_img.size[0] * self.scale), int(target_img.size[1] * self.scale)), Image.BICUBIC)
                    ...
                    if self.random_crop:
                        if self.random_scale and min_target_size > self.resolution:
                            if min_target_size < self.resolution:
                                ...
                                scale_size = random.randint(self.resolution, int(min_target_size) if min_target_size is not None else self.resolution)
        ...
        if self.random_crop:
            if self.random_scale and min_source_size > self.resolution:
                if min_source_size < self.resolution:
                    print_acc(
                        f"Unexpected values: min_source_size={min_source_size}, self.resolution={self.resolution}, image file={source_path}")
                    scale_size = self.resolution
                else:
                    scale_size = random.randint(self.resolution, int(min_source_size) if min_source_size is not None else self.resolution)
            ...
            source_img = transforms.RandomCrop(self.resolution)(source_img)
        else:
        ...
        if self.mask_map is not None:
            mask_path = self.mask_map.get(img_name, None)
            if mask_path is not None:
                ...
                # 生成masked_image：掩码区域设为0
                masked_image = source_tensor * (1 - mask_tensor)
            else:
                # 没有掩码时，全部保留
                mask_tensor = torch.zeros((1, self.resolution, self.resolution), dtype=source_tensor.dtype)
                masked_image = source_tensor
        ...
        # 确定返回的图像（优先使用target作为标准答案）
        final_image = target_tensor if target_tensor is not None else source_tensor
        
        # 返回数据
        if self.mask_map is not None:
            return {
                'image': final_image,  # 标准答案（target或source）
                'caption': prompt,
                'masked_image': masked_image,  # 带洞图像
                'mask': mask_tensor
            }
        else:
            if self.include_prompt:
